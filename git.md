...
# 쉬운듯 어려운듯 쉬운듯 어려운 git add와 git commit의 세계
* git status
<br>
 git은, 지속적으로 해당 폴더 내에 파일들의 '최종 수정전 상태'를 항상 기록하고 있다. 그리하여 내가 어떤 것을 수정하게 되고 저장 ctrl+s를 누르는 순간, 그것은 수정된것이 되고 git은 가장 마지막으로 수정되었던 상태와 현재 수정된 상태를 비교대조한다. 그리하여 조금이라도 변형이 일어나게 되면 이를 알아차리는데, 그 수정전 상태는 보통 쉽게 untracked 상태라고 하고, 방금 막 수정된 상태를 unmodified상태라고 한다. 그리고 이걸 확인할 수 있는 것이 git status이다.

 * git add
 <br>
  모든 파일은 수정이되는 순간 unmodified상태가 되는데,  unmodified상태가 modified 상태로 전환되고, 다시 modified가 staged 상태로 되어야 그 파일이 로컬 레파지토리 기억장소 어딘가에 짱박혀져서 '임시적으로' 저장될 수 있다.(그 저장된 것은 git push를 통해 결론적으로 깃헙측 레파지토리로 보내져 깃헙서버에 저장될 수 있게 된다.) <br>
그리하여, 파일의 unmodified에서 modified 상태로 신호를 켜주는 역할을  하는 것이 git add이다. git add를 쉽게 설명하자면, 수정으로 인해 바뀐 것을 '아, 내가 인지했다, 알고있다, 이걸 받아들이겠다'라는 신호를 깃에게 보내는 것이라고 대충 생각하면 된다. 보통, 수정된 파일 전부를 modified 상태로 켜주기 위해 <br> git add . <br> 를 많이 쓴다.

* git commit
<br>
그럼 그 modified로 켜진 상태의 파일을, staged 상태, 즉 게시 상태까지 해주는 명령어는 무엇일까? 바로 위에 쓴 명령어이다. git commit 명령어를 통해 stage 된 파일 내용들은, 로컬 레파지토리가 기억하게 된다. 그리하여 이 기억은 새롭게 커밋을 하기 전까지는 그대로 남은채 로컬 레파지토리에 남아 있게 된다.

* git push
<br>
그리하여 최종적을오 진짜 깃헙측 레파지토리로 보낼때, 즉 최종적으로 git commit 을통해 로컬 레파지토리 구석에서 기다리고 있던 파일 수정 내용들이 깃헙 서버를 통해 깃헙 디비로 보내지게 되는 것이다.
...
## 그러면, connect-youngjae 브랜치에 체크아웃 되어 있는 상태에서, 깃헙측 브랜치인 sangwoo 브랜치를 불러온다음, 수정을 하고 난 뒤, 이걸 hangyeol 깃헙 브랜치에 업로딩 할 수는 없을까요? 가능합니다! 실습해보세요!
