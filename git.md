...
그리하여, 파일의 unmodified에서 modified 상태로 신호를 켜주는 역할을  하는 것이 git add이다. git add를 쉽게 설명하자면, 수정으로 인해 바뀐 것을 '아, 내가 인지했다, 알고있다, 이걸 받아들이겠다'라는 신호를 깃에게 보내는 것이라고 대충 생각하면 된다. 그러면 깃은 add된 파일의 변화를 유의미한 것으로 받아들여서 기록을 남기려고 한다. 그것이 modified로 전환하는 것이다. 보통, 수정된 파일 전부를 modified 상태로 켜주기 위해 <br> git add . <br> 를 많이 쓴다.

* git commit
<br>
그럼 그 modified로 켜진 상태의 파일을, staged 상태, 즉 게시 상태까지 해주는 명령어는 무엇일까? 바로 위에 쓴 명령어이다. git commit 명령어를 통해 stage 된 파일 내용들은, 로컬 레파지토리가 담아서 기억하게 된다. 그리하여 이 기억은 새롭게 커밋을 하기 전까지는 그대로 남은채 브랜치별로 로컬 레파지토리에 남아 있게 된다.
